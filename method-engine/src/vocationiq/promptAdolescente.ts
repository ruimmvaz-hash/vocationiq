...
export function construirPromptAdolescente(
  ...
  const blocoCandidatasCatalogo = blocoCatalogoVocacional(catalogo, cursosPorDestino, yogas);
...
5. Onde entra a tua matéria nesta opção — a forma/função, nunca só o sector (correcção do especialista — bug real, RECORRENTE em duas rondas seguidas do mesmo relatório, sempre neste ponto exacto, em ambas as opções declaradas: saiu sistematicamente com só 2 frases, abaixo do mínimo dos outros 5 — ao contrário do ponto 4, este nunca tinha reforço próprio de profundidade, só "a forma/função" numa linha): desenvolve em pelo menos 3 frases — nomeia uma tarefa ou função concreta do dia a dia desta área onde a tua característica (um dom já nomeado nos pontos 1-4 ou em "Quem é") se aplicaria na prática, contrasta com outra tarefa ou função da MESMA área que NÃO encaixa tão bem no teu perfil (para mostrar que não é a área toda, é uma parte específica dela), e fecha ligando de volta ao dom ou traço que decide essa entrada. Este ponto mede-se pela mesma régua de profundidade dos outros 5 — nunca o mais curto do bloco.
6. Conclusão explícita — ver INSTRUCAO_VALIDACAO_OPCOES: uma das três frases-molde exactas (sustenta com clareza / sustenta parcialmente / não sustenta de forma natural), nunca omitida.`;
  ...
  const instrucaoCandidata = `Se a pool tiver 1 ou mais candidatas, primeiro escreve o bloco único "${MARCADORES.seleccaoCandidatas}" (ver INSTRUCAO_SELECCAO_CANDIDATAS). Depois, para cada candidata que passou o Passo 1 (ligação narrativa), um de dois formatos — em AMBOS, abre sempre com a frase de dom (ver INSTRUCAO_ABERTURA_CANDIDATAS: usa o FACTOR DE DOM já dado e traduzido) e NUNCA menciona duração de curso, trajecto ou via de entrada no corpo do texto (correcção do especialista — bug real: uma candidata escapou a esta regra sem citar nenhum número, só uma comparação relativa — "é a via mais longa de toda esta lista, e vale a pena teres isso presente antes de a levares muito a sério" — isto TAMBÉM é menção de percurso/duração, só que qualitativa em vez de numérica, e viola a regra na mesma: qualquer frase que compare, avalie ou comente a extensão, dificuldade ou exigência do PERCURSO de uma candidata contra as outras — "é a via mais longa", "é a mais rápida de todas", "exige mais anos que as outras" — pertence exclusivamente à linha "${MARCADORES.custoPrincipal}", nunca ao corpo do texto, mesmo sem números):
...
- PROIBIDO USAR A PALAVRA "CARTA": nunca escrevas "carta" (nem "mapa astral", "mapa natal") no texto do relatório — usa sempre "perfil". Correcto: "o teu perfil sustenta X". Proibido: "a tua carta sustenta X". Aplica-se a TODO o relatório (correcção do especialista — bug real: "de toda a tua carta" apareceu dentro de um bloco "${MARCADORES.candidata}", já longe desta regra), incluindo, em particular, o texto dentro de cada bloco "${MARCADORES.candidata}"/"${MARCADORES.grupo}" em "${SECCAO_TITULOS.candidataForaDaLista}".
...
- PROIBIDO: primeira pessoa do plural — REGRA GERAL, não uma lista fechada (correcção do especialista — bug real: "nomeámos" passou porque só havia exemplos, não a regra gramatical): qualquer verbo na 1ª pessoa do plural ("nós"), com terminação "-ámos"/"-emos"/"-imos" ou equivalente ("identificámos", "vimos", "calculámos", "nomeámos", "referimos", e qualquer outro, mesmo fora desta lista), é proibido — o relatório fala só da pessoa. Correcto: "o teu perfil mostra", "os dados indicam", "esta força já foi nomeada".
...
Para cada dimensão marcada EXTREMO (≤4 ou ≥7), a secção "${SECCAO_TITULOS.quemE}" tem de ter pelo menos uma frase que explique o que esse valor significa para esta pessoa especificamente — nunca deixar um extremo sem menção, tanto os ≤4 como os ≥7 (ver lembrete também na secção "${SECCAO_TITULOS.quemE}" abaixo).
...
RODA DA VIDA — LEMBRETE À PARTE (correcção do especialista — esta secção nunca mencionava as dimensões EXTREMO da Roda da Vida, apesar de a instrução completa estar nos dados técnicos "-- Roda da Vida --" acima): antes de escreveres a linha de síntese abaixo, confirma — CADA dimensão marcada "EXTREMO" nos dados técnicos (≤4 e ≥7) tem, algures nesta secção, pelo menos uma frase que nomeie o que esse valor significa para ti especificamente.
