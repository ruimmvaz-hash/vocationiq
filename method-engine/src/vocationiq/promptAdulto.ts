...
export const INSTRUCAO_SELECCAO_CANDIDATAS = `APRESENTAÇÃO DAS CANDIDATAS FORA DA LISTA — TECTO CALIBRADO PELA CLAREZA DECLARADA (recalibrado 22 Set, pedido do fundador — caso real: quando a pessoa já declara hipóteses em "${SECCAO_TITULOS.leituraPorOpcao}" (agora sempre pelo menos 1, nunca afectada por este tecto), a secção do catálogo, antes sem tecto nenhum, ainda somava mais 4-8 nomes a seguir — sobrecarga real para quem está indeciso, ruído desnecessário para quem já tem ideia clara. Os dados técnicos trazem "Tecto de apresentação em Candidata fora da lista" — um número já calculado a partir da clareza que a pessoa declarou (3, 4 ou 6) ou "sem tecto" (ramo adulto, ou registos antigos sem esse dado). NUNCA calcules este número tu próprio — usa sempre o valor exacto que vier nos dados técnicos): a secção "Candidatas do catálogo" traz a pool completa (Nível 1 ou 2), sem limite nenhum. A tua tarefa continua a ser decidir quais passam o filtro de ligação narrativa (Passo 1) — isso nunca muda; só a escolha de QUANTAS mostrar no fim é que agora tem tecto (ver PASSO 3B abaixo). Nunca inventar nenhuma candidata fora da pool; nunca omitir uma candidata elegível só para manter a secção curta ANTES de chegares ao tecto — o corte por causa do tecto só acontece no fim, depois de todas passarem o Passo 1 normalmente.
...
PASSO 3B — TECTO DE APRESENTAÇÃO E FRASE DE FECHO (ver dados técnicos, "Tecto de apresentação em Candidata fora da lista"): depois de decidido o Passo 1 (quem passa) e o Passo 3 (como se agrupam), se o número de destinos a apresentar (cada membro de um grupo conta individualmente; um grupo de 3 conta 3) ultrapassar o tecto, apresenta só os de maior soma de pesos (mesma ordem do Passo 2) até ao tecto. NUNCA partir um grupo a meio — um grupo do Passo 3 entra inteiro ou fica todo de fora do tecto, nunca um membro sim e outro não (se o grupo inteiro não couber, fica de fora do tecto por inteiro). Quando isto deixar destinos genuínos (que passaram o Passo 1) de fora só por causa do tecto, fecha a secção "${SECCAO_TITULOS.candidataForaDaLista}" com UMA frase curta e honesta — nunca um bloco novo, nunca a explicação astrológica completa — nomeando-os, ex.: "O teu perfil também toca, de forma mais ligeira, em [nomes] — não aprofundados aqui para manteres o foco nas pistas mais fortes." NUNCA escrever esta frase quando nada foi cortado pelo tecto (nunca inventar um corte que não existiu), e NUNCA incluir nela uma candidata que reprovou o Passo 1 — só as que passaram e ficaram de fora só pelo tecto. Sem tecto nos dados técnicos = comportamento antigo, apresenta todas as que passam, sem frase de fecho nenhuma. Este tecto aplica-se SÓ a esta secção — a secção "${SECCAO_TITULOS.leituraPorOpcao}" mostra sempre TODAS as opções que a pessoa declarou, sem excepção nenhuma.
Nunca escolhas só pela contagem de camadas (convergência) — usa sempre a soma de pesos para ordenar.
Antes de qualquer bloco "${MARCADORES.grupo}" ou "${MARCADORES.candidata}", escreve OBRIGATORIAMENTE um bloco único "${MARCADORES.seleccaoCandidatas}" (machine-readable, nunca omitido), explicando em prosa curta: (a) a que dom já nomeado em "${SECCAO_TITULOS.quemE}" cada candidata que passou o Passo 1 se liga, (b) quais candidatas da pool completa (nomeia-as pelo nome, com a sua soma de pesos) reprovaram o Passo 1 e porquê, (c) como as que passaram ficaram agrupadas — que grupos, com que nomes, e quais ficaram individuais, (d) se consideraste alguma "Alternativa pelo perfil" para o Passo 1B — nomeia-a mesmo quando a resposta é não promover (ex.: "considerei X, tem Nível 2, mas a ligação a QUALQUER dom nomeado seria genérica demais, não promovo"); se não havia nenhuma alternativa com Nível 1/2 sequer, di-lo também, numa frase; (e) se o tecto do Passo 3B cortou alguma candidata ou grupo que passou o Passo 1 — nomeia-os e confirma que ficaram na frase de fecho da secção, mesmo quando a resposta é que nada foi cortado. Este bloco nunca aparece no relatório entregue ao cliente — é só para auditoria interna do raciocínio.
