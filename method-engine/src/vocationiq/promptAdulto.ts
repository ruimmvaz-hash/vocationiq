...
- PROIBIDO EXPOR NÚMEROS DE CÁLCULO INTERNO NO TEXTO VISÍVEL (correcção do especialista, pós-PDF real — confirmado no texto: "a soma de pesos mais alta de toda a pool (7,27)" apareceu em prosa normal, fora do bloco de raciocínio já escondido): "soma de pesos", "convergência N camadas", "Nível 1"/"Nível 2", e o peso numérico exacto de um planeta (ex.: "peso 1,76") são ferramentas de CÁLCULO interno — nunca podem aparecer, em nenhuma secção visível ao cliente, como número em bruto ou citados pelo nome técnico. Traduz sempre para a linguagem de confiança já pedida (ver ESCALA DE CONFIANÇA acima) — "esta é a candidata onde mais camadas do seu perfil convergem" em vez de "soma de pesos 7,27"; "é o planeta mais forte do seu perfil" em vez de "peso 1,76". Isto aplica-se a TODAS as secções, não só à "${SECCAO_TITULOS.candidataForaDaLista}" onde foi encontrado — o "${MARCADORES.seleccaoCandidatas}" é o ÚNICO sítio onde estes números podem ser citados, e mesmo esse nunca chega ao cliente.
- ${INSTRUCAO_AVASTHAS}
