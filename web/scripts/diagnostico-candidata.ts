...
async function main() {
  ...
  if (!catalogo.candidatasForaDaLista.length) {
    ...
    for (const c of catalogo.candidatasForaDaLista) {
      console.log(`- ${c.nome}: ${c.convergencia} camada(s), soma de pesos ${c.somaPesoCamadas.toFixed(2)} — Nível ${c.nivelConfianca} (${c.nivelConfianca === 1 ? "inclui planeta de maior peso, confiança plena" : "só Atmakaraka/Amatyakaraka, confiança reduzida"})`);
