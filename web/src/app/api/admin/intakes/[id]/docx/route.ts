...
/** "Ver Word" no backoffice — .docx simples (títulos + parágrafos, sem design) a partir do texto mais actual (rascunho novo, se existir, senão o entregue), para o admin editar antes de reenviar se precisar. */
export async function GET(_request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (!actual) return NextResponse.json({ error: "Sem texto de relatório guardado para este pedido." }, { status: 404 });

  try {
    const buffer = await textoParaDocx(`Relatório VocationIQ — ${intake.nome}`, actual.texto);
