...
import { construirPromptAdolescente, construirPromptAdulto, blocoVargottama, paragrafoAntiDaParaTudo, planetaPt, type VocationiqIntakeAdulto } from "@naveya/method-engine";
import { construirPromptCritica, construirPromptCriticaAdolescente, construirBlocosPromptCritica, construirBlocosPromptCriticaAdolescente, parseCritica, construirPromptReescrita, removerBlocosOpcaoNaoAutorizados, verificarContagemBlocosOpcao, TOTAL_CRITERIOS_ADULTO, TOTAL_CRITERIOS_ADOLESCENTE, verificarProfundidadeLeituraPorOpcao, combinarFalhasComGuardas, verificarPalavraCarta, verificarPrimeiraPessoaPlural, verificarYogaDeGrupo, verificarCandidatasElegiveisPresentes, verificarJustificacaoRejeicaoCandidatas, verificarNomeacaoConjuncoes, verificarYogasReforcoGeralNomeados, verificarLinguagemReservaNivel2, verificarFraseVargottama, verificarOrdemParagrafoAntiDaParaTudo } from "@/lib/criticaRelatorio";
...
export async function POST(request: Request) {
  ...
  try {
    ...
    // Critério 16 (verificarFraseVargottama) — mesma lógica do ramo
    // adulto, derivada da mesma função determinística `blocoVargottama`.
...
export async function PATCH(request: Request) {
  ...
  if (acao === "reescrever") {
    ...
    try {
      ...
      const { intakeAdolescente, catalogoResultados, d1 } = await calcularDadosAstrologicosAdolescente(intake);
      ...
      // não fica guardada no rascunho — mesmo padrão já usado aqui para
      // `intakeAdolescente.opcoesAdolescente` (critério 34).
      const ehPos12 = intakeAdolescente.anoEscolaridade === "pos-12";
      const totalCriteriosEsperado = ehPos12 ? TOTAL_CRITERIOS_ADULTO : TOTAL_CRITERIOS_ADOLESCENTE;
